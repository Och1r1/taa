...
- [ ] Add Supabase Auth sign-in flow, initially using anonymous sign-in or magic links.
- [ ] Create a `profiles` table for display names and optional avatar metadata.
- [ ] Associate room hosts and players with authenticated user IDs.
- [ ] Replace host-token-only authorization with Auth-aware host checks.
- [ ] Preserve the current host token only as a temporary migration fallback, then remove it.
- [ ] Restrict RLS reads and writes to the relevant room participants.
- [ ] Ensure an answer can only be submitted by its authenticated player.

### Acceptance checks

- [ ] Refreshing a page restores the same signed-in identity.
- [ ] A player cannot heartbeat, answer for, or remove another player.
- [ ] Only the authenticated host can start, reveal, advance, finish, or close a room.
- [ ] Existing public-PIN flow continues to work during the migration.
